...
script_hash=$(md5sum "$source_file" | cut -d ' ' -f 1)
...
do
    ...
    fh=$(md5sum "$file" | cut -d ' ' -f 1)
    sh=$(md5sum "$source_file" | cut -d ' ' -f 1)
    ph=$(md5sum "$password_file" | cut -d ' ' -f 1)

    if [[ $file_lock -eq 1 && ! $fh == $file_hash ]]
    then
        echo "Restoring original file after change"
        echo "$file_data" > "$file"
    fi

    if [[ $script_lock -eq 1 && ! $sh == $script_hash ]]
    then
    ...
    if [[ $password_lock -eq 1 && ! $ph == $password_file_hash ]]
    then
